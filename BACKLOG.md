...
### PACK — Gaucho Studio v2: evolução do IDE (fila aprovada em 2026-08-13, em curso)

Contexto: em 2026-08-12/13 o Studio virou Python-only (modo Local TS/JS removido), ganhou splitters, console interativo com stdin (+ flush de prompt), token do workspace em sessionStorage e Ctrl+Enter — tudo entregue, validado e no ar (ver diário do AGENTS.md). Anders aprovou a fila seguinte, a executar pelo trilho essential-workflows, um bundle por vez:
...
3. **Aba Notebook (estilo Jupyter)** — ✅ CONCLUÍDO 2026-08-13: entregue conforme o desenho fechado. Abrir `.ipynb` troca a superfície do editor pela view de células (`StudioNotebook.tsx`): células code em Monaco python compacto (auto-altura, Ctrl+Enter executa, FIM com prefixo concatenando as células de código anteriores via `getLeadingContext` novo no autocompleteProvider) e markdown renderizadas com toggle de edição; outputs texto/stream/traceback/PNG inline; add/remover célula; header com status do kernel + Interromper/Reiniciar. Server: `studio-kernel-bridge.py` (helper `jupyter_client` fora da jail, JSON por stdin/stdout, validado vivo 8/8) + `studioNotebookKernel.ts` (unit `gaucho-studio-kernel-*` na jail, idle-kill 30 min, RuntimeMaxSec=8h, interrupt via SIGINT, varredura de connection files órfãos) + 4 rotas gated (`notebook/stream|execute|interrupt|shutdown`). Persistência nbformat v4.5 (`notebookFormat.ts`, parse/serialize + reducers). TDD: 41 testes novos (530 total); smoke Playwright em build de produção (estado entre células, PNG do matplotlib, outputs persistidos no arquivo, restart → NameError); deploy + health local/público 200. Desenho original (mantido como referência): células de código+markdown numa superfície própria do Studio. Decisões de Anders: (a) motor = **ipykernel real** (ipykernel + jupyter_client instalados no venv da jail; protocolo Jupyter dá estado persistente entre células, rich output e interrupt limpo — sem embutir o Jupyter server/UI inteiro); (b) formato = **`.ipynb` padrão nbformat v4** no workspace (outputs persistem no arquivo, abre em VS Code/Jupyter/Colab; escolhido após esclarecer que o autocomplete independe do formato — o FIM opera no Monaco de cada célula); (c) saída rica v1 = **texto + imagens PNG** (stdout/stderr/result repr + display_data image/png em base64; sem HTML arbitrário na v1). DECISÕES do Claude (vetáveis): UI segue o padrão do markdown preview — abrir um `.ipynb` troca a superfície do editor pela view de células (cada célula é um Monaco python com o autocompleteProvider atual, e o prefix do FIM concatena as células anteriores pra sugestão ciente do notebook inteiro); ciclo de vida do kernel herda o padrão do terminal — 1 kernel por vez na mesma jail (`systemd-run`), idle-kill ~30 min sem execução, `MemoryMax=1G`, backstop `RuntimeMaxSec`, botão de restart do kernel na UI; transporte reusa SSE p/ saída + POST p/ ações. Spike ✅ (2026-08-13, 7/7 PASS): kernel na jail via `systemd-run` + connection file em `/workspace`; helper `jupyter_client` FORA da jail conecta via loopback (jail não tem PrivateNetwork — ZMQ atravessa), estado persiste entre células, matplotlib PNG chega em `display_data` base64, erro estruturado com traceback, shutdown limpo via protocolo (a unit morre sozinha). Arquitetura confirmada (plano A leve, sem gateway HTTP): node ⇄ helper python (`jupyter_client`, JSON por stdin/stdout, mesmo padrão do runner) ⇄ kernel na jail; browser segue falando só SSE+POST com o Next. Dependências instaladas e pinadas no venv: `ipykernel==7.3.0` (+ `jupyter-client 8.9.1`) e `matplotlib==3.10.5`. Falta: plano TDD (helper protocolizado, manager de kernel com idle-kill, rotas, nbformat v4 I/O, UI de células com Monaco+FIM por célula).

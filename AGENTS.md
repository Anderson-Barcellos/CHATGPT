...
- O assistente contextual do Studio segue somente-leitura: o painel lateral pode usar `web_search_preview` e preserva status/citações; a assistência por célula permanece sem tools. A chamada não sobrescreve reasoning nem verbosity, deixando os defaults do modelo.

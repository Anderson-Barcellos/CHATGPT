# Diário do agente — Gaúcho Chat (append-only)

Histórico operacional movido de `AGENTS.md` em 2026-09-11, verbatim, para o `AGENTS.md` caber no teto de 32 KiB que o Codex aplica ao arquivo do projeto (antes disso ele só lia até maio de 2026). Novas entradas entram **no fim** deste arquivo, com o template `### YYYY-MM-DD HH:MM - Título` seguido de `Context:`, `Details:` e `Notes:`. Se uma entrada antiga conflitar com o bloco Estado Atual do `AGENTS.md`, o `AGENTS.md` é a fonte mais recente. Duas entradas (2026-08-20 e 2026-09-03) estavam acima do título do `AGENTS.md` e foram recolocadas na ordem cronológica.

## Pontos de atenção registrados em 2026-05 (não triados; frentes vivas estão no `BACKLOG.md`)
...
## Rodadas iniciais (histórico de 2026-04/05)
...
### 2026-09-03 15:12 - Fechamento do repositório fechado por Anders
...
Details:
O Studio mobile foi ocultado no ProductNav abaixo de 768 px em `9d3fe30`. L1 foi auditado na worktree, commitado em `80ee904` e integrado por `ad285eb`; Memory V2 E1/E2 foi integrada por `ea9eda7`, mantendo `MEMORY_V2_ENABLED` desligada, autoridades JSON/SQLite exclusivas e sem migração real. Artefatos locais não versionados foram movidos recuperavelmente para `/root/CHATGPT-quarentena-2026-09-03/closeout/`; `.gitignore` passou a bloquear caches Python em `ad655ed`. Os erros React do SoundCase foram reproduzidos por testes e corrigidos em `5d6233a`; o teto dos testes duráveis de worker foi calibrado em `e278a3e`. DOMPurify, PDF.js e overrides de fflate/nanoid foram atualizados em `46f0437`; a projeção intencional de campos privados e a imagem autenticada foram formalizadas no lint em `1ac197b`. Documentação e estado operacional foram reconciliados em `d29c07d` e `f00c5cb`.
...
### 2026-09-07 - SoundCase SC3: configurações persistentes e Realtime pelo player

Context: Anders aprovou configurações próprias do SoundCase acessíveis no acervo e antes da geração, mais início de Realtime para uma versão salva sem criar outro job TTS.

Details: `useSoundCaseSettings` compartilha preferências na página/painel via `gaucho-soundcase:settings:v1`, com hidratação SSR consistente, normalização e fallback de sessão se o storage falhar. `DirectionSidebar` mantém formato independente da direção automática; voltar a automático limpa overrides de voz/ritmo/instruções. `SoundCasePlayer` inicia Realtime, apresenta erro por versão e preserva a troca explícita de fonte; seek fica associado ao arquivo. O hook carrega o snapshot pela nova rota autenticada `GET .../versions/[versionId]/source` (private/no-store), dentro da mesma cerca de cancelamento da sessão, inclusive no fluxo Gerar e ouvir agora. Não usar o draft atual ao reler versões antigas.

Notes: evidências e estado da SC3 em BACKLOG.md. Build e Chrome isolados de B2–B6 em `/root/.cache/soundcase-sc3-review-rd4pjy52`; nenhum dado privado ou provider foi acionado pelo QA. O teste de navegador usa a rota real de snapshot e WAV sintético servido pela rota real com Range; WebRTC/provider são simulados. Chrome precisa de pasta temporária acessível aos subprocessos (`TMPDIR=/var/tmp` funcionou; `/root/.cache` provocou crash por permissão). Não publicar o checkout inteiro como parte desta entrega.

Fechamento 2026-09-07: Anders aprovou a revisão e a SC3 foi marcada como fechada no BACKLOG. Build preservada para publicação futura; este aceite não publicou, commitou nem enviou alterações ao remoto.

Publicação 2026-09-09: na retomada de Anders, publicada a build SC3 `ZvnMJP226jtiS-Y0va9fo` em `.next`, preservando SC2 em `/root/.cache/chatgpt-next-before-sc3-20260909T042041Z` e 450 assets antigos. Serviço retomado com health local/público 200; Chrome público desktop/mobile confirmou settings persistentes, botão Realtime e `/source` autenticado 200/no-store, sem gerar áudio pago. WIP B2–B6 continua fora da build; commit/push permanecem pendentes. Evidências e retorno em BACKLOG.md e `/root/.cache/sc3-deploy-result.json`.

### 2026-09-11 - P8: polimento geral do Gaucho Chat

Context: Anders aprovou o polimento conservador do Chat e pediu retirar do desktop os atalhos duplicados de Chat/SoundCase/Studio, já cobertos pela rail, além de reduzir levemente a barra mobile.

Details: `.gc-chat-ui` escopa Lexend, escala tipográfica e movimento curto também nos portals; o composer mobile usa uma faixa de controles de 45 px preservando alvos de 44 px. Stores do shell/painéis usam seletores Zustand e `MessageBubble` é memoizado. A rail fecha ao selecionar a conversa já ativa. O snapshot `.next-before-sc2-*` foi excluído da descoberta Tailwind porque gerava CSS com bytes inválidos. P5 já havia removido os atalhos duplicados do header; P8 preservou e testou esse contrato.

Notes: P8 pronta para revisão, sem commit/push/publicação/restart. Validação: 170 arquivos/842 testes, TypeScript, lint sem erros, build e `git diff --check`; Chrome sintético desktop/mobile claro/escuro, Settings/command/rail, breakpoints 320/375/768/1024 sem overflow, 44 px e reduced motion. Produção e dados privados permaneceram intactos; capturas e snapshot prévio estão registrados no BACKLOG.

Atualização 2026-09-11: após referência móvel real de Anders, a faixa do composer passou a ser sempre `nowrap`, 41 px no total, com controles de 36 px, enviar de 40 px, seletor limitado a 8 rem e superfície/borda/texto mais claros apenas nessa área. Chrome em 390×844 e 320×844 confirmou menu funcional, altura estável, zero overflow/console; suíte completa 170 arquivos/842 testes, TypeScript, lint sem erros e build passaram. Anders pediu reset dos serviços: todas as units Chat/Pulse/SoundCase foram reiniciadas antes do ajuste e, ao final, `chatgpt.service` recebeu a build validada com health local/público 200. P8 segue pronta para revisão, sem commit/push nem mudança no Apache.

### 2026-09-12 - Compactação mobile de mensagens, menus e faixa do composer

Context: Anders aprovou o plano de reduzir espaços/caixas no celular sem diminuir letras; esclareceu durante a implementação que queria eliminar a rolagem horizontal da faixa de modelos, com redução de aproximadamente 10%.

Details: balões mobile com largura conforme o conteúdo e padding 8 × 10 px; menus de modelo/raciocínio compactados por classe própria em portal, modelo com largura 224 px e margens laterais de 12 px. Controles 36 → 33 px, envio 40 → 36 px, teto do seletor 128 → 116 px. DECISÃO: o grupo de controles passa a quebrar linha quando necessário, substituindo o scroll horizontal sem esconder funções. Alterados CSS, composer/container, dois testes existentes e documentação; tipografia, desktop e WIP anterior preservados.

Notes: 842 testes/170 arquivos passaram; tsc e build aprovados; lint zero erros e um warning anterior; build mantém warning anterior de tracing do Studio. Chrome/Playwright 8/8 cenários, com 48 capturas antes/depois, geometria desktop e fontes/entrelinhas conferidas, zero overflow da página/faixa. Evidências e harness apontados no BACKLOG. O dev server reteve CSS antigo, então o QA final usou build isolada; um teste que fixava a largura antiga do seletor foi atualizado e a suíte completa repetida. A cópia não carrega dados privados, segredos ou o boot que encerra units do Studio; chamadas de API do navegador foram simuladas. Sem publicação, commit, push ou restart de produção. Pronta para revisão.

### 2026-09-12 - Densidade mobile de 92% sem zoom

Context: Anders mostrou a interface real ainda grande em 100% e aprovou reproduzir a proporção visual de zoom 92% por tokens/componentes, no Chat mobile inteiro, com composer em uma linha a partir de 390 px.

Details: tokens mobile reduziram geometria e tipografia com pisos de leitura; textarea ficou em 16 px e alvos coarse-pointer em 40 px. Header, navegação, welcome, mensagens, metadados, quick actions, menus e composer foram compactados. O composer usa largura orçada por controle, trunca somente o modelo e quebra em duas linhas abaixo de 390 px, sem `overflow-x-auto`. A barra de oito ações ganha a coluna do avatar no mobile para caber em uma linha em 390 px; a de sete ações não muda de alinhamento. Nenhum `zoom` ou `transform: scale` foi introduzido.

Notes: 170 arquivos/843 testes, tsc, build isolada e diff-check passaram. QA Chrome touch/coarse-pointer: 12/12 em 320/390/430/767/768/1280 px, claro/escuro, sem overflow ou pageerror; 390 px ou mais fica em uma linha, 320 px em duas. O lint completo falhou somente ao analisar o backup compilado não versionado `.next-before-sc2-*` (PRE_EXISTING_FAILURE); excluindo esse artefato, zero erros e um warning anterior. Evidência reduzida a 572 KB em `/root/.cache/chat-mobile-density-92-evidence`; cópia de build removida. Sem produção, provider, dados privados, commit ou push.

Publicação 2026-09-12: Anders autorizou o restart; como o CSS novo depende de artefato compilado, foi feita a build do checkout atual e reiniciado `chatgpt.service`. Build `d4RKONEnNLmGGtDQDtGUA` ativa; anterior `PULk1bmsSMh4DkcifsDMz` preservada em `/root/.cache/chatgpt-next-before-density92-20260912T2148Z`. Após a janela transitória de aproximadamente dois segundos, health local e público ficaram `healthy`. O chunk CSS com `--gc-mobile-touch-target:2.5rem` respondeu 200 local e publicamente. Apache, dados privados, commit e push não foram tocados.

### 2026-09-13 - Reatividade mobile para iPhone 17 Pro Max

Context: Anders pediu boas práticas de reatividade e fluência no iPhone 17 Pro Max e suspeitou de redundâncias nos polimentos mobile anteriores. A auditoria encontrou quatro lacunas reais (teclado do iOS sem `visualViewport`, paisagem caindo no layout de tablet, safe-area lateral ausente, blur e `layout` animation por balão) e sujeira inofensiva (classe sem CSS, variantes redundantes, tokens duplicados/mortos, fallbacks triplos). Bloco aprovado sem tocar densidade, cores ou desktop.

Details: `MOBILE_MEDIA_QUERY` em `lib/layout/breakpoints.ts` unifica a definição de mobile (largura ≤ 767 px ou paisagem curta com toque); `useIsMobile` e o variant `md` do Tailwind (redefinido em `app/globals.css`) seguem a mesma condição. `hooks/useVisualViewport.ts` grava `--gc-visual-viewport-height` e `data-keyboard-open` no `<html>` e é montado em `WorkspaceFrameV2`. CSS mobile: shell seguindo o visual viewport, dock sem safe-area inferior com teclado aberto, `gc-safe-x/left/right`, balão sem `backdrop-filter`, `overscroll-behavior: contain`, `touch-action: manipulation` sob `pointer: coarse`. `MessageBubble` desliga `layout` no mobile. DECISÃO: paisagem curta usa o layout mobile inteiro; SoundCase e Studio intocados; alvo de 40 px mantido por decisão anterior.

Notes: TDD red→green com `hooks/useIsMobile.test.ts`, `hooks/useVisualViewport.test.ts` e contratos em `app/globals.visual.test.ts` e `WorkspaceLayoutV2.test.tsx`. Gates na cópia isolada `/root/.cache/chat-mobile-iphone-20260913` (node_modules por hardlink; symlink derruba o Turbopack; dev server exige origem `localhost` e reinício para recompilar CSS). Evidência antes/depois em `/root/.cache/chat-mobile-iphone-evidence`, harness em `/root/.cache/chat-mobile-iphone-qa.mjs`. Teclado do iOS não é emulável: validar no aparelho após deploy. Resultados completos em `TEST_LOGS.md`. Sem commit, push, publicação ou restart.
...
Details: `MessageBubble` deixou de aplicar `gc-message-quick-actions-wide`, e o CSS mobile perdeu a expansão e a margem negativa associadas. Permanecem `fit-content`, `max-width: 100%`, `flex-wrap`, sete ações fixas, `Regenerar` como oitava ação condicional e alvos de 40 px. O contrato visual cobre contenção e ausência do seletor legado. DECISÃO: o QA usou DOM sintético em `page.setContent()` com o CSS real da build isolada; nenhuma segunda instância Next foi iniciada, pois o boot chamaria `stopOrphanedStudioUnits()` por wildcard e poderia encerrar sessões Studio vivas.

Notes: commit de comportamento `f446849`. Validação final: 172/172 arquivos e 861/861 testes, TypeScript, lint fonte com zero erros e um warning anterior, build `/chat` isolada com 41/41 páginas e `git diff --check`. O lint bruto registrou a falha preexistente do snapshot compilado (4.446 erros/88.384 warnings) antes da repetição excluindo somente esse caminho. Chrome/Playwright aprovou 10/10 cenários em 320/390/430/767 e 956×440, claro/escuro, com quebra apenas em 320, oito ações em uma linha em 390/430, alvos 40×40 px e zero overflow/erro. Evidências em `/root/.cache/chat-mobile-actions-qa-uiiYST`. Sem servidor temporário, provider, dados privados, deploy, restart, push ou alteração no Apache.

### 2026-09-21 — Grok 4.7 e leitura Realtime no SoundCase

Context: Anders aprovou substituir usos ativos do GPT-5.4 mini por Grok 4.7/medium e testar voz Realtime xAI apenas no SoundCase, preservando Luna default e o TTS/arquivo OpenAI. Entrega isolada em `.worktrees/grok-chat-soundcase`, branch `codex/grok-chat-soundcase`, plano `f2d0658` e implementação `d452d83`.

Details: adapter Responses xAI com stream/nonstream, memória e bridge de imagem OpenAI; aliases legados sem reescrever históricos; Documento/Deepsearch com executor local persistido, cancelamento e recuperação de interrupção sem repetir cobrança; Pulse/Studio/extrações integrados. SoundCase recebe seleção explícita de engine/voz/velocidade, token temporário e leitura do snapshot imutável, sem microfone. `GAUCHO_ISOLATED_RUNTIME=true` protege units Studio durante QA. DECISÃO: aceitar `GROK_API_KEY` como alias server-side de `XAI_API_KEY`; configuração do shell não foi copiada para o serviço. Integração texto escalada de Terra para Sol após lacunas demonstradas; voz permaneceu com Terra. Revisões independentes e responsabilidades registradas no relatório.

Notes: 183 arquivos/928 testes, TypeScript, lint (zero erros, um warning anterior), build `/chat` isolada 41/41 e 34 verificações Playwright desktop/mobile aprovados. Correções intermediárias de tipos/imports e fixture browser estão registradas com exits reais. Voz real: catálogo de 28 vozes, áudio inicial em 2029 ms, 6.85 s e transcrição fiel do trecho sintético, custo estimado US$ 0.0131. Duas chamadas reais de texto responderam 200, mas recusaram a resposta literal `PRONTO`; esse smoke falhou, permanecendo ressalva explícita antes da aceitação/publicação. Evidências em `/root/.cache/grok-chat-soundcase-qa`, registro consolidado em `docs/plans/2026-09-21-grok-chat-soundcase-report.md`. Instância QA encerrada, checkout de produção limpo e preservado. Sem merge, push, deploy, restart de produção, alteração de proxy ou dados privados. Estado: pronta para revisão local, fechamento com Anders.

### 2026-09-21 — Grok publicado para revisão do Anders

Anders autorizou publicação e restart após perguntar se o serviço já havia sido reiniciado. `main` recebeu fast-forward até `bc8dd1d` e a build validada `raY9v6DYHbbAxp2R0PApb` foi instalada com backup recuperável e retenção de 17 assets antigos. Credencial xAI provisionada em `/etc/chatgpt/xai.env` (root:root 0600), por drop-in do serviço, sem expor o valor ou alterar `.env.production`. Fonte do drop-in versionada; `daemon-reload` e `systemctl restart chatgpt.service` concluídos com exit 0. Não havia sessões Studio ativas.

Health local/público 200/healthy, rotas de voz anônimas 401 e catálogo real autenticado 200 com 28 vozes/no-store. As 34 verificações Playwright desktop/mobile passaram também na URL pública, usando sessão efêmera e APIs/áudio sintéticos. Sem geração adicional paga ou acesso a conteúdo pessoal. Evidências e backup em `/root/.cache/grok-deploy-20260922`; registro detalhado no relatório da entrega. APACHE.md atualizado, sem mudar vhost, porta, CSP ou cookies; sem reload Apache e sem push. Entrega publicada, ainda pronta para revisão; ressalva de obediência do Grok no smoke textual mantida.

### 2026-09-22 — Review de segurança e operação implementado, sem publicação

Codex: Anders aprovou os sete ajustes do review, instância única e login obrigatório em produção. Implementação na worktree `codex/review-seguranca-operacao`, base `0adcfe3`; plano aprovado `3cbeea7`. Auth/health (`ef390d5`), runtime/Studio (`83390ea`) e ferramentas operacionais (`afbb378`) ficaram em commits locais. Domínio `sonaris.us` e migração permaneceram fora.

Produção sem auth configurada e segundo processo sobre o mesmo armazenamento agora encerram no boot. Readiness não recupera nem cria dados; liveness é separado. Novas sessões Studio têm proprietário comprovado e vínculo de lifecycle, sem stop wildcard; legadas ficam para transição explícita. Pré-deploy e QA recusam checkout vivo, a unit não mata ocupante da porta e Pulse passa token pelo stdin.
...
### 2026-09-25 — Composer revisto em faixa única

Codex: Anders corrigiu a escolha: Pesquisa virou lupa na barra única; depois pediu remover o botão “+” de anexos e aprovou visualmente o resultado (“Bah ficou trrrrri”). DECISÃO: remover o seletor manual de arquivo e seu input, preservando colagem de imagem, arrastar/soltar e anexos existentes. DECISÃO: o menu de modelo deslocava o frame `overflow-hidden` por rolagem horizontal programática (`composerX=-64`); `overflow: clip` corrigiu para `composerX=1` no fluxo real, sem alterar o scroll interno da conversa.

Gates finais na worktree: 194 arquivos/990 testes, TypeScript, lint (0 erros, 1 warning anterior), build isolado de 42 páginas e `git diff --check` aprovados. Chrome/Playwright com autenticação sintética: 320/390/430, paisagem curta e desktop, temas dark/light, Sol/Pro, menu Pesquisa e envio inteiro; sem overflow, recorte ou `pageerror`. Evidências e detalhes em `docs/plans/2026-09-24-composer-mobile-stt-report.md`. Produção, dados privados, serviço, Apache, merge e push preservados.

...
O layout é híbrido. `components/soundcase/SoundCaseWorkspace.tsx` concentra estado e composição, e recebe `variant`: `"page"` mantém o grid de três colunas com dock e sheets da rota; `"panel"` monta coluna única rolante com Direção e Acervo como `Collapsible` inline (nunca Sheet aninhado) e a ação de gerar num rodapé acima da safe-area. `SoundCaseShell` é o invólucro da rota — header com voltar ao Chat sempre visível no mobile — e `SoundCasePanel` é o Sheet lateral direito montado em `GauchoChatShellV2`, no mesmo padrão de Configurações e do painel contextual. No mobile o botão do `ConversationRailV2` abre o painel em vez de navegar (o `href` continua valendo para acesso direto e nova aba); no desktop navega para a rota. A paleta `--sc-*` é mapeada nos tokens `gc-*`, de modo que a superfície acompanha light/dark — o "papel" bege do editor é identidade preservada. Enquanto uma edição ainda não foi confirmada pelo servidor, uma cópia de recuperação fica no `localStorage` do mesmo navegador e é removida após o CAS, descarte remoto ou exclusão; beacon/keepalive na saída é apenas uma tentativa adicional, não a fonte de durabilidade.

A sessão Realtime vive fora do painel. `components/soundcase/SoundCaseRealtimeProvider.tsx` chama `useSoundCaseRealtime()` uma única vez e publica a sessão por contexto; a rota monta o provider dentro de `SoundCaseShell`, e o chat monta em `GauchoChatShellV2` como irmão de `SoundCasePanel`. Sem isso a conexão WebRTC morreria junto com o Sheet, que o Radix desmonta ao fechar. `SoundCaseWorkspace` consome `useSoundCaseRealtimeSession()` — que lança se não houver provider acima — e `components/soundcase/SoundCaseRealtimeBar.tsx` dá o controle persistente (status, reabrir o painel, parar) enquanto a leitura toca com o painel fechado. Fechar o painel preserva o áudio, mas desmonta `useSoundCase`: o draft e a versão selecionada são refeitos por fetch ao reabrir.

O estado de cada geração chega à interface por um contrato único: `describeSoundCaseVersion()` em `lib/soundcase/progress.ts` converte `status`, `progress` e `audio` em `{ tone, label, ratio, playable }` — `audio_ready` é tone `partial` (arquivo tocável, capa em curso) e o status cru nunca vaza para a tela. O acervo (`SoundCaseLibrary`) renderiza cada versão como cartão com capa desde o início da geração (placeholder "Criando capa" até a capa existir); o cartão selecionado expande resumo e metadados, então não existe cartão de resultado separado. O acervo também sabe o que está soando: `useSoundCaseRealtime` expõe a `versionId` da sessão e `SoundCasePlayer` reporta `onPlaybackChange`, e o workspace deriva `playback: { versionId, source: "realtime" | "file" }` para marcar "Tocando · Realtime" ou "Tocando · arquivo". Enquanto alguma versão não selecionada estiver ativa, `useSoundCase` recarrega só o projeto a cada 4 s (`getSoundCaseProjectPollInterval`) sem tocar na seleção do usuário.

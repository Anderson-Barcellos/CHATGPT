...
**Status de execução (2026-08-07, sessão Claude):** Tasks 1–7 concluídas com TDD e commits por fatia — `7c90676` provisionamento + jaula provada, `aef6b80` step-up auth, `db1735c` file API, `005ef47` zip lifecycle, `50c0bc5` runner SSE, `6bec0a5` cliente modo Python (controller puro `lib/studio/serverWorkspace.ts` + hook fino + shell com alternância/modal/zip), `7351cc1` FIM em python. Gates atuais: 462 testes, tsc, lint, build e `git diff --check` limpos. Task 8 concluída em 2026-08-07 13:10: senha no env, restart, prova viva da sandbox e smoke Playwright em produção — frente encerrada, sem push. `PRE_EXISTING_FAILURE`: `npm audit --omit=dev` acusa `pdfjs-dist` GHSA-hq66-cqwq-w95j (advisory nova, independente desta frente; fix é major 6.x — decisão de Anders fora deste escopo).
...
- [x] **Step 1:** gates completos: `npm test` (462), `npx tsc --noEmit`, `npm run lint`, `npm run build`, `npm audit --omit=dev` (só o PRE_EXISTING pdfjs-dist), `git diff --check`.
- [x] **Step 2:** senha fornecida por Anders, gravada em `.env.production` (backup `.env.production.bak-20260807`), restart do `chatgpt.service`, health local e público 200; antes do restart o status respondia como app de hoje (rollback provado).
- [x] **Step 3 (prova viva):** `prova.py` real: import local ok, log gravado (dono `studio`), OpenAI respondeu com a chave herdada do env, escrita em `/root` (PermissionError) e `/etc` (OSError) negadas; Stop matou loop infinito (`aborted`, 4 s); roundtrip salvar → resetar → restaurar byte a byte (sha256 idêntico). Timeout: coberto pela prova viva de `RuntimeMaxSec` da Task 1 + testes do runner (decisão: não re-provar com env curto para evitar restarts extras).
- [x] **Step 4:** smoke autenticado em produção via Playwright: unlock pela UI, árvore real, `main.py` auto-aberto, run com SSE ao vivo (268 ms), autosave UI→disco, ghost text FIM em python observado, zip salvo e baixado pelo browser. Negativos por curl: 401 sem cookie, `studio_workspace_locked` sem token, senha errada rejeitada.
- [x] **Step 5:** docs canônicos atualizados (API/ARCHITECTURE/INFRASTRUCTURE/BACKLOG/CLAUDE + diário AGENTS.md), commits coerentes `6bec0a5`→`28068f7`. Sem push (aguarda pedido de Anders).
